...
const LookupStarByID = async () => {
  const instance = await StarNotary.deployed();
  const lookupid = document.getElementById('lookupStarId').value;
  console.log('lookupid:', lookupid);
  let lookupName = await instance.lookUptokenIdToStarInfo(lookupid);
  console.log('lookupName:', lookupName);
  App.setStatus(
    'the name of Star is ' + lookupName + '. with id:' + lookupid + ' '
  );
};
//
